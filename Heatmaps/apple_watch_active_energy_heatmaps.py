...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Apple Watch active energy heatmaps.")
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Directory for plots and summary CSVs. Defaults to Heatmaps/Graphs/ActiveEnergyBurned.",
    )
    parser.add_argument(
        "--timezone",
        default="US/Pacific",
...
def main() -> None:
    ...
    out_dir = Path(args.output_dir).expanduser().resolve() if args.output_dir else Path(__file__).resolve().parent / "Graphs" / "ActiveEnergyBurned"
    ensure_dir(out_dir)
    run_heatmap_suite(
        root=root,
        output_dir=out_dir,
